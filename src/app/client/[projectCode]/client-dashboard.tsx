...
export function ClientDashboard({ project }: Props) {
  ...
  const totalPaid = (project.payment_records || []).reduce(
      ...
      <main className="flex-1 pt-24 pb-16 px-4 relative z-10">
        <div className="max-w-4xl mx-auto space-y-10">
              ...
              <div className="flex items-center gap-4">
                {project.logo_url ? (
                  <img
                    ...
                    alt="Logo"
                    className="w-12 h-12 rounded-md border border-white/10 bg-white/5 object-contain p-1"
                  />
                ) : (
                  <div className="w-12 h-12 rounded-md border border-dashed border-white/10 flex items-center justify-center">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className="text-[9px] text-white/20 uppercase font-bold">
                      ...
                      {
                        SERVICE_TYPES.find(
                          (s) => s.value === project.service_type,
                        )?.label
                      }
                    </span>
                  </div>
          ...
          <section className="space-y-4">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30 px-1">
              Project Roadmap
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
              {statusSteps.map((step, i) => {
                ...
                return (
                  <div
                    key={step.value}
                    className={`p-3 rounded-md border text-center transition-all ${isCurrent ? "bg-primary/5 border-primary/40" : isDone ? "bg-white/5 border-white/10 opacity-70" : "bg-transparent border-white/5 opacity-20"}`}
                  >
                    <div
                      className={`w-6 h-6 rounded-md flex items-center justify-center mx-auto mb-2 ${isCurrent ? "bg-primary text-black" : "bg-white/5 text-white"}`}
                    >
                      {isDone && !isCurrent ? (
                    ...
                    <p
                      className={`text-[8px] font-bold uppercase tracking-widest ${isCurrent ? "text-primary" : "text-white/60"}`}
                    >
                      {step.label}
                    </p>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="p-6 rounded-md border border-white/5 bg-white/[0.01] space-y-3">
                <h3 className="text-[9px] font-bold uppercase tracking-widest text-white/20">
                  Brief
                </h3>
              ...
              <div className="space-y-4">
                <h3 className="text-[9px] font-bold uppercase tracking-widest text-white/20 px-1 flex items-center gap-2">
                  <History className="w-3.5 h-3.5" /> Financial History
                </h3>
                <div className="rounded-md border border-white/5 bg-white/[0.01] overflow-hidden">
                              ...
                              <td className="px-4 py-3 text-center">
                                {inv ? (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center gap-1 px-2 py-1 rounded bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-all"
                                  >
                  ...
                  {(project.payment_records || []).length === 0 && (
                    <p className="p-8 text-center text-white/20 text-xs italic">
              ...
              {project.final_price > 0 && (
                <div className="p-6 rounded-md border border-white/5 bg-white/[0.02] space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className="text-[8px] rounded-sm bg-white/5 text-white/40">
                      {
                        PAYMENT_STATUSES.find(
                          (ps) => ps.value === project.payment_status,
                        )?.label
                      }
                    </Badge>
                  </div>
                ...
                <div className="space-y-1">
                  {project.project_files?.map((f) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-white/50 group-hover:text-white truncate">
                        {f.file_name}
                      </span>
                    </a>
                  ))}
